...
class NNModel:
    ...
    def view_batch(self):
        ...
        data = next(iter(self.trainloader)) # getting first batch        
        
        y = data[1].view(8,8).numpy() # extracting labels
        
        # extracting 8x8 tiling of the batch images
        x = data[0]
        x = torch.reshape(x, (x.shape[0], x.shape[2], x.shape[3]))
        x = x.permute(0, 2, 1)
        x = torch.reshape(x, (8, 28*8, 28*1))
        x = x.permute(0, 2, 1)
        x = torch.reshape(x, (28*8, 28*8)).numpy()
        """
        # alternative way of concatenting images in rows
        temp_list = []
        for i in range(0, 64, 8):
            img = torch.clone(data[0][i][0]).detach()
            for j in range(i+1, i+8):
                img1 = torch.clone(data[0][j][0]).detach()
                img = torch.cat((img, img1), 1)
            temp_list.append(img)
        
        # concatenating rows in final array 8x8 images
        final_img_list = temp_list[0]
        for i in range(1, 8):
            img_row = temp_list[i]    
            final_img_list = torch.cat((final_img_list, img_row), 0)
        final_img_list = final_img_list.numpy() 
        """
        return x, y
